...
class Seq2SeqActor(nn.Module):
    def __init__(self, node_state_dim, vnf_state_dim, num_nodes, max_sfc_length, hidden_dim, num_layers):
        ...
        self.reliability_fc = nn.Linear(1, vnf_state_dim)  # batch_size * 1 * vnf_state_dim

        self.net_fc = nn.Linear(node_state_dim, hidden_dim)
        self.embedding = nn.Linear(vnf_state_dim, hidden_dim)   # input: batch_size * (max_sfc_length + 2 + 1) * vnf_state_dim
        ...
        self.fc_out = nn.Linear(hidden_dim, num_nodes)  # output: batch_size * (max_sfc_length + 2 + 1) * num_nodes
    ...
    def forward(self, state):
        ...
        sfc = torch.cat((sfc_state, source_dest_node_pair, reliability_requirement),
                        dim=1)  # batch_size * (max_sfc_length + 2 + 1) * vnf_state_dim

        embedded = self.embedding(sfc)  # batch_size * (max_sfc_length + 2 + 1) * hidden_dim
        encoder_input = torch.cat((embedded, batch_net_state), dim=1)   # batch_size * (max_sfc_length + 2 + 1 + num_nodes) * hidden_dim
        ...
        logits = self.fc_out(decoder_outputs[:, :self.max_sfc_length, :])   # batch_size * max_sfc_length * num_nodes
        probs = F.softmax(logits, dim=-1)
        return logits, probs
...
class DecoderActor(nn.Module):
    ...
    def forward(self, state):
        ...
        encoder_output, encoder_hidden_state = self.encoder(sfc)  # (max_sfc_length + 2 + 1) * batch_size * embedding_dim, 1 * batch_size * embedding_dim
